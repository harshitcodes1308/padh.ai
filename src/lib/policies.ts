export const POLICIES = [
    {
        slug: "privacy-policy",
        title: "Privacy Policy",
        content: `📄 Privacy Policy – Saviours AI
Last Updated: 29th January 2026

Saviours AI ("we", "our", "us") operates a digital education and AI-powered learning platform built specially for CBSE students ("Service").

Your privacy matters to us. This Privacy Policy explains what information we collect, how we use it, and how we keep it safe when you use our website, app, or services.

1. Information We Collect

We collect only what is necessary to run and improve our platform.

a) Personal Information
When you sign up or use our services, we may collect:
- Name
- Email address
- Phone number
- School and grade
- Account login details
- Billing details (handled securely by Razorpay)

We do not store your card, UPI, or bank details on our servers.

b) Usage Information
To understand how our platform is used, we may collect:
- IP address
- Device and browser type
- Pages visited and time spent
- Interaction with AI features

c) Uploaded Content
If you upload questions, documents, or images to our AI tools, we process them temporarily to generate responses and improve learning quality. We do not permanently store this content unless required for service functionality.

d) Cookies
We use cookies to:
- Keep you logged in
- Improve user experience
- Analyze performance
- Prevent misuse and fraud

You can disable cookies anytime through your browser settings.

2. How We Use Your Information

We use your information to:
- Provide and manage Saviours AI services
- Create and maintain user accounts
- Process subscriptions and payments
- Deliver educational content
- Improve AI accuracy and platform features
- Provide customer support
- Send important service updates
- Prevent fraud or misuse
- Comply with legal requirements

3. Payment Processing

All payments are securely processed by Razorpay.
We never collect or store sensitive payment information such as card numbers, CVV, or UPI PINs.

Razorpay’s own privacy policies apply to all payment transactions.

4. Sharing of Information

We may share limited data only with trusted partners such as:
- Payment processors like Razorpay
- Cloud hosting providers
- Analytics and performance tools
- Email or SMS communication services
- Legal authorities if required by law

We do not sell your personal data to anyone.

5. Data Security

We use reasonable technical and organizational safeguards to protect your data from:
- Unauthorized access
- Loss or misuse
- Alteration or disclosure

That said, no online system can guarantee complete security.

6. Data Retention

We keep your information only for as long as needed:
- To provide our services
- To meet legal obligations
- To resolve disputes
- To enforce our terms

You may request account deletion at any time by contacting us.

7. Children’s Privacy

Saviours AI is made for school students.
If you are under 18, you should use our services with permission and guidance from a parent or guardian.

8. Your Rights

You have the right to:
- Access your personal data
- Request corrections
- Delete your account
- Withdraw consent

You can contact us at:
care.savioursai@gmail.com

9. Third-Party Links

Our platform may contain links to third-party websites. We are not responsible for their content or privacy practices.

10. Changes to This Policy

We may update this Privacy Policy from time to time. Any changes will be posted on this page with an updated date.

11. Contact Us

If you have any questions or concerns about this Privacy Policy, feel free to reach out:

Saviours AI
Email: care.savioursai@gmail.com`
    },
    {
        slug: "terms-and-conditions",
        title: "Terms & Conditions",
        content: `📄 Terms & Conditions – Saviours AI
Last Updated: 29th January 2026

Welcome to Saviours AI ("we", "our", "us"). These Terms & Conditions explain the rules for using our website, applications, and AI-powered education services ("Service").

By accessing or using Saviours AI, you agree to follow these Terms. If you do not agree, please do not use the Service.

1. About the Service

Saviours AI is a digital learning platform created for CBSE students. Our services include:
- AI-powered doubt solving
- Notes and revision tools
- Custom tests
- Study planners
- Academic assistance and guidance

The Service may be offered through free features as well as paid subscription plans.

2. Eligibility

- Users must be at least 13 years old to use the Service.
- If you are under 18, you confirm that you have permission from a parent or legal guardian.
- You agree to provide accurate and complete information during registration.

3. Account Responsibilities

You are responsible for:
- Keeping your login details confidential
- All activity that happens through your account
- Informing us immediately if you notice unauthorized access

We reserve the right to suspend or restrict accounts suspected of misuse or policy violations.

4. Subscriptions & Payments

- All payments are processed securely through Razorpay.
- Prices are displayed on our website and may include applicable taxes.
- Subscription duration, features, and renewal details are shown on the pricing or checkout page.
- We may update pricing or plans with prior notice.

5. Refunds & Cancellations

Refunds and cancellations are governed by our separate Refund & Cancellation Policy, which is available on our website.
Please review that policy carefully before making a purchase.

6. Acceptable Use Policy

You agree not to:
- Misuse or manipulate the AI system
- Upload harmful, illegal, misleading, or abusive content
- Attempt to reverse engineer or disrupt the platform
- Bypass or misuse payment systems
- Share accounts with others
- Use the Service for cheating, plagiarism, or academic misconduct
- Violate any applicable laws or regulations

Violation of these rules may result in suspension or termination of your account.

7. Intellectual Property

All content, software, logos, designs, and AI tools available on Saviours AI are owned by us or our licensors.

You may not:
- Copy, reproduce, or redistribute content
- Sell or resell access to the Service
- Modify platform content
- Use our branding without written permission

8. Service Availability

We aim to provide a smooth and reliable experience, but we do not guarantee:
- Continuous or uninterrupted access
- Error-free operation
- Specific academic results or outcomes

The Service may be temporarily unavailable due to maintenance, updates, or technical issues.

9. Limitation of Liability

To the maximum extent permitted by law:
- Saviours AI is not responsible for academic results or performance outcomes
- We are not liable for data loss, indirect damages, or service interruptions
- Use of the Service is at your own discretion and risk

10. Termination

We may suspend or terminate access to the Service if:
- These Terms are violated
- Fraud, abuse, or misuse is detected
- Required by law or legal authorities

You may stop using the Service at any time.

11. Governing Law & Jurisdiction

These Terms are governed by the laws of India.
Any disputes will be subject to the jurisdiction of courts located in Lucknow, Uttar Pradesh, India.

12. Changes to These Terms

We may update these Terms occasionally. Continued use of the Service after changes means you accept the revised Terms.

13. Contact Us

If you have any questions or concerns regarding these Terms, you can contact us at:

Saviours AI
Email: care.savioursai@gmail.com`
    },
    {
        slug: "refund-policy",
        title: "Refund & Cancellation Policy",
        content: `📄 Refund & Cancellation Policy – Saviours AI
Last Updated: 29th January 2026

At Saviours AI, we aim to deliver high-quality digital learning experiences. This Refund & Cancellation Policy explains when cancellations or refunds may be allowed.

1. Nature of Service

Saviours AI provides digital and subscription-based educational services such as:
- AI-powered learning tools
- Notes and revision material
- Tests and assessments
- Study planners and academic support

Since these services are digital and usually delivered instantly or shortly after purchase, refunds are limited and subject to the conditions below.

2. Cancellation by User

- You may cancel your subscription at any time through your account dashboard or by contacting our support team.
- Cancellation will stop future billing cycles.
- Cancellation does not automatically qualify for a refund for the current active subscription period, unless stated otherwise in this policy.

3. Refund Eligibility

Refunds may be considered only in the following situations:
- Duplicate payment made by the user
- Technical issues that result in no access to the Service for an extended period
- Incorrect amount charged
- Access not provided after successful payment

All refund requests must be submitted within 7 days from the date of transaction.

4. Non-Refundable Cases

Refunds will not be provided in the following cases:
- Change of mind after using the Service
- Partial use of a subscription
- Dissatisfaction with academic outcomes
- Failure to cancel before automatic renewal
- Violation of our Terms & Conditions

5. How to Request a Refund

To request a refund, please email us at:
care.savioursai@gmail.com

Include the following details:
- Registered email address or phone number
- Transaction ID
- Payment date
- Clear reason for the refund request

Our team will review your request and respond within 5 to 7 working days.

6. Refund Processing Timeline

If your refund request is approved:
- The refund will be processed to the original payment method via Razorpay.
- The amount may take 7 to 10 business days to reflect, depending on your bank or wallet provider.

7. Cancellation by Saviours AI

We reserve the right to suspend or cancel accounts in cases involving:
- Fraudulent activity
- Abuse or misuse of services
- Legal or regulatory violations
- Breach of our Terms & Conditions

In such cases, refunds may be issued at our discretion.

8. Changes to This Policy

We may update this Refund & Cancellation Policy from time to time. Any changes will be posted on this page along with an updated revision date.

9. Contact Us

For any refund or cancellation related questions, you can contact us at:

Saviours AI
Email: care.savioursai@gmail.com`
    },
    {
        slug: "delivery-policy",
        title: "Delivery Policy",
        content: `📄 Delivery Policy – Saviours AI
Last Updated: 29th January 2026

This Delivery Policy explains how access to digital services and products on Saviours AI is provided after a successful payment.

1. Nature of Delivery

Saviours AI offers digital-only educational services, including:
- AI-powered learning tools
- Notes and revision material
- Customized tests
- Study planners

No physical products are shipped or delivered.

2. Delivery Method

Once your payment is successfully confirmed through Razorpay, access to the Service is provided in one of the following ways:
- Direct access through your Saviours AI account dashboard, or
- Login credentials or access confirmation sent to your registered email address

3. Delivery Timeline

- Most purchases provide instant or near-instant access.
- In rare cases, service activation may take up to 24 hours due to technical or verification reasons.

4. Failed or Delayed Delivery

If you do not receive access after payment, please try the following:
- Check your registered email address, including spam or promotions folders
- Log out and log back into your account
- Wait a short while in case of delayed activation

If the issue still persists, contact us at:
care.savioursai@gmail.com

We aim to resolve delivery-related issues as quickly as possible.

5. Access Issues

If you face technical problems while accessing or using the Service after delivery, please contact our support team so we can investigate and fix the issue promptly.

6. Policy Updates

We may update this Delivery Policy from time to time. Any changes will be posted on this page along with an updated revision date.

7. Contact Us

For any delivery-related questions or concerns, you can contact us at:

Saviours AI
Email: care.savioursai@gmail.com`
    },
    {
        slug: "contact-us",
        title: "Contact Us",
        content: `📄 Contact Us – Saviours AI

We’re here to support you throughout your learning journey with Saviours AI.
If you have questions about our services, subscriptions, payments, refunds, or face any technical issues, feel free to reach out to us.

📌 Business Information

Business Name: Saviours AI
Email: care.savioursai@gmail.com

🕘 Support Hours

Monday to Saturday: 10:00 AM to 7:00 PM IST
Sunday: Closed

📬 How to Reach Us

- Email us for quick and reliable assistance
- Use the in-app support or contact form on the platform, if available

🔒 Data & Payment Safety

- All payments are processed securely through Razorpay
- We do not store any card, bank, or UPI details on our servers

🌐 About Saviours AI

Saviours AI is an AI-powered educational platform built especially for CBSE students. We help learners with:
- Instant doubt solving using AI
- Clear and concise revision notes
- Custom tests and assessments
- Smart study planners for better time management

We’re always happy to help and improve your learning experience.`
    }
];
