...
const PDF_PATH = "/Users/harshitsingh/Documents/padhai-cbse/public/pdfs/competency/AI/AI All years.pdf";
const OUTPUT_PATH = "/Users/harshitsingh/Documents/padhai-cbse/src/data/precision-ai.ts";
...
async function main() {
  if (!fs.existsSync(PDF_PATH)) {
    console.error(`AI PDF not found at: ${PDF_PATH}`);
    process.exit(1);
  }
  ...
  unique.forEach((q, i) => {
    q.id = `ai-q-${i + 1}`;
    q.subject = "Artificial Intelligence";
  });

  console.log(`Successfully parsed ${unique.length} unique AI questions.`);
  ...
  const tsContent = `// Auto-generated Artificial Intelligence Precision Questions
import { PrecisionQuestion } from "./precision-config";

export const aiQuestions: PrecisionQuestion[] = ${JSON.stringify(unique, null, 2)};
`;
