...
# Verify if there are any broken events (e.g. empty fields, wrong options size)
broken = []
for ev in events:
    if not ev["year"] or not ev["title"] or not ev["importance"] or not ev["recall_question"]["question"] or len(ev["recall_question"]["options"]) != 4:
        broken.append(ev)

if broken:
    print(f"Warning: Found {len(broken)} broken events!")
    print(json.dumps(broken, indent=2))
else:
    print("All events parsed successfully and verified!")

# Write TS content
ts_content = "export const CHRONO_DATA = " + json.dumps(events, indent=4) + ";\n"

output_path = "/Users/harshitsingh/Documents/padhai-cbse/src/data/chrono-config.ts"
