...
for (let i = 0; i < lines.length; i++) {
  ...
  if (line.startsWith('# CHAPTER')) {
    ...
    currentPYQ = {
      year: 2024, // Default, will try to parse
      question: '',
      steps: [],
      finalAnswer: ''
    };
    ...
    currentTopic.pyqs.push(currentPYQ);
    state = 'NONE';
    ...
    if (state === 'FORMULA' && currentTopic) {
      ...
      if (line !== '') {
        let stepLine = line;
        if (line === '```latex' || line === '```') return; // skip latex ticks in solution steps, or just keep them
        // Actually let's keep them if we use latex rendering
        if (currentPYQ) {
...
chapters.forEach(c => {
  c.topics.forEach(t => {
    ...
    // If no solved example was found in the text directly, let's just make the first PYQ the solved example
